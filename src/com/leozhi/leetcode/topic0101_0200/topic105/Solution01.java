package com.leozhi.leetcode.topic0101_0200.topic105;
...
public class Solution01 {
    ...
    public TreeNode buildTree(int[] preorder, int[] inorder) {
        ...
        return recursive(preorder, 0, n - 1, 0);
    }
     ...
     * @return 根节点
     */
    private TreeNode recursive(int[] preorder, int preStart, int preEnd, int inStart) {
        ...
        // 左子树前序遍历的结束位置为根节点的位置加上左子树长度，即 leftPreEnd
        int leftPreEnd = preStart + leftLen;
        // 左子树中序遍历的开始位置为当前中序遍历的开始位置，即 inStart
        root.left = recursive(preorder, preStart + 1, leftPreEnd, inStart);
        // 右子树前序遍历的开始位置为左子树前序遍历的结束位置加 1 ,即 leftPreEnd + 1
        // 右子树前序遍历的结束位置为当前前序遍历的结束位置，即 preEnd
        // 右子树中序遍历的开始位置为根节点在中序遍历中的位置加 1 ，即 index + 1
        root.right = recursive(preorder, leftPreEnd + 1, preEnd, index + 1);
        return root;
    }
}